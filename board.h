...
public:
	...
	// bool top_row_right_stuck(){
	// 	for (int i = 0; i < 3; i++){
	// 		if ( tile[0][i] != 0 && tile[0][i+1] == 0)
	// 			return false;
	// 		if ( tile[0][i] >= 3 && tile[0][i] == tile[0][i + 1])
	// 			return false;
	// 		if ( tile[0][i] != 0 && tile[0][i] + tile[0][i + 1] == 3)
	// 			return false;
	// 	}
	// 	return true;
	// }

	// bool left_col_down_stuck(){
	// 	for (int i = 0; i < 3; i++){
	// 		if (tile[i][0] != 0 && tile[i+1][0] == 0)
	// 			return false;
	// 		if ( tile[i][0] >= 3 && tile[i][0] == tile[i + 1][0])
	// 			return false;
	// 		if ( tile[i][0] != 0 && tile[i][0] + tile[i + 1][0] == 3)
	// 			return false;
	// 	}
	// 	return true;
	// }
	...
	/**
	 * calculate reward based on diff between every two adjacent tiles
	 * return the reward
	 */
	// reward adjacency_reward(){
	// 	int each_reward[4] = {1, 1, 0, -1}; // reward of {== && >3, 1&2, 0, others}
	// 	reward score = 0;
	// 	for( int r = 0; r < 4 ; r++){
	// 		for (int c = 0; c < 4; c++){
	// 			if (c + 1 < 4) {
	// 				if ( tile[r][c] == 0 || tile[r][c + 1] ==0 )
	// 					score += each_reward[2];
	// 				else if ( tile[r][c] == tile[r][c + 1] && tile[r][c] >= 3)
	// 					score += each_reward[0];
	// 				else if ( tile[r][c] + tile[r][c + 1] == 3 && tile[r][c] != 0 )
	// 					score += each_reward[1];
	// 				else
	// 					score += each_reward[3];
	// 			}
	// 			if (r + 1 < 4) {
	// 				if ( tile[r][c] == 0 || tile[r+1][c] ==0 )
	// 					score += each_reward[2];
	// 				else if ( tile[r][c] == tile[r+1][c] && tile[r][c] >= 3)
	// 					score += each_reward[0];
	// 				else if ( tile[r][c] + tile[r+1][c] == 3 && tile[r][c] != 0 )
	// 					score += each_reward[1];
	// 				else
	// 					score += each_reward[3];
	// 			}
